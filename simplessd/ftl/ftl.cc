...
FTL::FTL(ConfigReader &c, DRAM::AbstractDRAM *d) : conf(c), pDRAM(d) {
  ...
  switch (conf.readInt(CONFIG_FTL, FTL_MAPPING_MODE)) {
    case PAGE_MAPPING:
      ...
      break;
    case WL_PAGE_MAPPING:
      pFTL = new WLPageMapping(conf, param, pPAL, pDRAM);
      break;
    case RBPA_PAGE_MAPPING:
      pFTL = new RBPAPageMapping(conf, param, pPAL, pDRAM);
      break;
    case LA_PAGE_MAPPING:
      pFTL = new LAPageMapping(conf, param, pPAL, pDRAM);
      break;
    default:
      panic("Unsupported FTL mapping mode %d",
            (int)conf.readInt(CONFIG_FTL, FTL_MAPPING_MODE));
      break;
  }
