...
DCAMapping::DCAMapping(ConfigReader &c, Parameter &p, PAL::PAL *l,
                       DRAM::AbstractDRAM *d)
    : AbstractFTL(p, l, d),
      pPAL(l),
      conf(c),
      ...
      freeBlocksPerUnit(param.pageCountToMaxPerf, 0),
      bReclaimMore(false) {
  ...
  for (uint32_t i = 0; i < param.totalPhysicalBlocks; i++) {
    ...
    freeBlocksPerUnit.at(i % param.pageCountToMaxPerf)++;
  }
...
void DCAMapping::read(Request &req, uint64_t &tick) {
  // uint64_t begin = tick;
...
void DCAMapping::write(Request &req, uint64_t &tick) {
  // uint64_t begin = tick;
...
void DCAMapping::trim(Request &req, uint64_t &tick) {
  // uint64_t begin = tick;
...
void DCAMapping::format(LPNRange &range, uint64_t &tick) {
...
Status *DCAMapping::getStatus(uint64_t lpnBegin, uint64_t lpnEnd) {
...
float DCAMapping::freeBlockRatio() {
  return (float)nFreeBlocks / param.totalPhysicalBlocks;
}

uint32_t DCAMapping::convertBlockIdx(uint32_t blockIdx) {
  return blockIdx % param.pageCountToMaxPerf;
}

uint32_t DCAMapping::getFreeBlock(uint32_t idx) {
  ...
  if (nFreeBlocks > 0) {
    ...
    uint32_t targetUnit = getBestParallelUnit();
    auto iter = freeBlocks.begin();

    for (; iter != freeBlocks.end(); iter++) {
      blockIndex = iter->getBlockIndex();

      if (blockIndex % param.pageCountToMaxPerf == targetUnit) {
        break;
      }
    }
    ...
    // [DCA 핵심 추가 로직]
    // 할당된 블록이 속한 병렬 유닛(채널)의 남은 프리 블록 개수를 1 감소시킴
    // 이를 통해 getBestParallelUnit() 함수가 항상 최신 상태를 기반으로 판단 가능
    freeBlocksPerUnit.at(blockIndex % param.pageCountToMaxPerf)--;
  }
  else {
...
uint32_t DCAMapping::getLastFreeBlock(Bitset &iomap) {
  if (!bRandomTweak || (lastFreeBlockIOMap & iomap).any()) {
    // 기존의 단순 순차 증가(lastFreeBlockIndex++)를 제거하고,
    // 현재 시점에서 가장 여유로운 병렬 유닛(채널/웨이)을 탐색하여 할당
    // lastFreeBlockIndex = getBestParallelUnit();
    lastFreeBlockIndex++;
...
uint32_t DCAMapping::getBestParallelUnit() {
  uint32_t bestIndex = 0;
  uint32_t maxFreeBlocks = 0;

  // 모든 병렬 유닛(채널/웨이)을 순회하며 가장 여유로운 유닛을 탐색
  for (uint32_t i = 0; i < param.pageCountToMaxPerf; i++) {
    if (freeBlocksPerUnit.at(i) > maxFreeBlocks) {
      maxFreeBlocks = freeBlocksPerUnit.at(i);
      bestIndex = i;
    }
  }

  // 만약 모든 유닛의 프리 블록이 0이라면 GC가 시급한 상태
  // 여기서는 우선 탐색된 인덱스(0)를 반환하고 이후 FTL 로직에 의해 GC 트리거
  return bestIndex;
}

// calculate weight of each block regarding victim selection policy
void DCAMapping::calculateVictimWeight(
...
void DCAMapping::selectVictimBlock(std::vector<uint32_t> &list,
                                   uint64_t &tick) {
...
void DCAMapping::doGarbageCollection(std::vector<uint32_t> &blocksToReclaim,
                                     uint64_t &tick) {
  PAL::Request req(param.ioUnitInPage);
...
void DCAMapping::readInternal(Request &req, uint64_t &tick) {
...
void DCAMapping::writeInternal(Request &req, uint64_t &tick, bool sendToPAL) {
  PAL::Request palRequest(req);
...
void DCAMapping::trimInternal(Request &req, uint64_t &tick) {
  auto mappingList = table.find(req.lpn);
...
void DCAMapping::eraseInternal(PAL::Request &req, uint64_t &tick) {
  ...
  if (erasedCount < threshold) {
    ...
    freeBlocksPerUnit.at(req.blockIndex % param.pageCountToMaxPerf)++;
  }
...
float DCAMapping::calculateWearLeveling() {
...
void DCAMapping::calculateTotalPages(uint64_t &valid, uint64_t &invalid) {
  valid = 0;
  invalid = 0;
...
void DCAMapping::getStatList(std::vector<Stats> &list, std::string prefix) {
  Stats temp;

  temp.name = prefix + "dca_mapping.gc.count";
  temp.desc = "Total GC count";
  list.push_back(temp);

  temp.name = prefix + "dca_mapping.gc.reclaimed_blocks";
  temp.desc = "Total reclaimed blocks in GC";
  list.push_back(temp);

  temp.name = prefix + "dca_mapping.gc.superpage_copies";
  temp.desc = "Total copied valid superpages during GC";
  list.push_back(temp);

  temp.name = prefix + "dca_mapping.gc.page_copies";
  ...
  temp.name = prefix + "dca_mapping.wear_leveling";
  temp.desc = "Wear-leveling factor";
  list.push_back(temp);
}

void DCAMapping::getStatValues(std::vector<double> &values) {
...
void DCAMapping::resetStatValues() {
  memset(&stat, 0, sizeof(stat));
}
