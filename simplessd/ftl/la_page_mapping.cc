...
#include "ftl/la_page_mapping.hh"
...
LAPageMapping::LAPageMapping(ConfigReader &c, Parameter &p, PAL::PAL *l,
                         DRAM::AbstractDRAM *d)
    : AbstractFTL(p, l, d),
      pPAL(l),
      conf(c),
  ...
  // Test-scenario only: pre-wear a fraction of blocks (config-gated, no-op by
  // default). Does not alter the page-mapping algorithm itself.
...
LAPageMapping::~LAPageMapping() {}

bool LAPageMapping::initialize() {
...
void LAPageMapping::read(Request &req, uint64_t &tick) {
  uint64_t begin = tick;
...
void LAPageMapping::write(Request &req, uint64_t &tick) {
  uint64_t begin = tick;
...
void LAPageMapping::trim(Request &req, uint64_t &tick) {
  uint64_t begin = tick;
...
void LAPageMapping::format(LPNRange &range, uint64_t &tick) {
...
Status *LAPageMapping::getStatus(uint64_t lpnBegin, uint64_t lpnEnd) {
...
float LAPageMapping::freeBlockRatio() {
  return (float)nFreeBlocks / param.totalPhysicalBlocks;
}

uint32_t LAPageMapping::convertBlockIdx(uint32_t blockIdx) {
  return blockIdx % param.pageCountToMaxPerf;
}

uint32_t LAPageMapping::getFreeBlock(uint32_t idx) {
  ...
  if (nFreeBlocks > 0) {
    // ── Load-Adaptive: free 블록의 채널별 평균 eraseCount 계산 → 마모 낮은
    //    채널 우선 배치(고마모 채널 회피). 평균이 동률이면 요청된 idx를 유지해
    //    슈퍼페이지 병렬성을 보존한다. ──
    uint32_t numCh = param.pageCountToMaxPerf;
    std::vector<float> chSumErase(numCh, 0.f);
    std::vector<uint32_t> chCount(numCh, 0);

    for (auto &fb : freeBlocks) {
      uint32_t ch = fb.getBlockIndex() % numCh;
      chSumErase[ch] += fb.getEraseCount();
      chCount[ch]++;
    }

    uint32_t targetCh = idx;
    float minAvg = (chCount[idx] > 0)
                       ? chSumErase[idx] / chCount[idx]
    ...
    for (uint32_t ch = 0; ch < numCh; ch++) {
      ...
      float avg = chSumErase[ch] / chCount[ch];
    ...
    // Search block which is blockIdx % param.pageCountToMaxPerf == targetCh
    auto iter = freeBlocks.begin();

    for (; iter != freeBlocks.end(); iter++) {
      blockIndex = iter->getBlockIndex();

      if (blockIndex % param.pageCountToMaxPerf == targetCh) {
        break;
      }
    }
    ...
    // Device end-of-life: every spare block has worn past the erase threshold.
    // Record the final wear state and stop cleanly (instead of a hard panic) so
    // baseline and improved runs are directly comparable. This branch only
    // fires at exhaustion and does not influence any wear-leveling decision.
    ...
    printf("\n*** DEVICE END-OF-LIFE *** algo=la_page_mapping gc=%" PRIu64
           " host_write_pages=%" PRIu64
           " dead_blocks=%" PRIu64 " / %" PRIu64 " @tick %" PRIu64 "\n",
           stat.gcCount, stat.hostWritePages, nDead,
...
uint32_t LAPageMapping::getLastFreeBlock(Bitset &iomap) {
...
void LAPageMapping::calculateVictimWeight(
  ...
  // ── Load-Adaptive: 채널별 평균 eraseCount 계산 ──
  uint32_t numCh = param.pageCountToMaxPerf;
  std::vector<float> chAvgErase(numCh, 0.f);
  std::vector<uint32_t> chBlockCount(numCh, 0);

  for (auto &iter : blocks) {
    uint32_t ch = iter.first % numCh;
    chAvgErase[ch] += iter.second.getEraseCount();
    chBlockCount[ch]++;
  }
  for (auto &fb : freeBlocks) {
    uint32_t ch = fb.getBlockIndex() % numCh;
    chAvgErase[ch] += fb.getEraseCount();
    chBlockCount[ch]++;
  }

  float totalAvgErase = 0.f;
  uint32_t totalChCount = 0;
  for (uint32_t ch = 0; ch < numCh; ch++) {
    if (chBlockCount[ch] > 0) {
      chAvgErase[ch] /= chBlockCount[ch];
      totalAvgErase += chAvgErase[ch];
      totalChCount++;
    }
  }
  if (totalChCount > 0) totalAvgErase /= totalChCount;
  ...
  switch (policy) {
    ...
    case POLICY_DCHOICE:
      for (auto &iter : blocks) {
        ...
        float validWeight = (float)iter.second.getValidPageCountRaw();

        // Load-Adaptive Channel Balancing (α=0.1): 고마모 채널의 블록 가중치를
        // 낮춰 우선 GC 대상으로 삼아 erase를 분산
        uint32_t ch = iter.first % numCh;
        float chBias = 0.f;
        if (totalAvgErase > 0.f) {
          chBias = (chAvgErase[ch] - totalAvgErase) / totalAvgErase;
        }
        float alpha = (float)param.pagesInBlock *
                      conf.readFloat(CONFIG_FTL, FTL_LA_ALPHA_SCALE);
        float adjustedWeight = validWeight - chBias * alpha;

        weight.push_back({iter.first, adjustedWeight});
      }
...
void LAPageMapping::selectVictimBlock(std::vector<uint32_t> &list,
                                    uint64_t &tick) {
...
void LAPageMapping::doGarbageCollection(std::vector<uint32_t> &blocksToReclaim,
                                      uint64_t &tick) {
  PAL::Request req(param.ioUnitInPage);
...
void LAPageMapping::readInternal(Request &req, uint64_t &tick) {
...
void LAPageMapping::writeInternal(Request &req, uint64_t &tick, bool sendToPAL) {
  PAL::Request palRequest(req);
...
void LAPageMapping::trimInternal(Request &req, uint64_t &tick) {
  auto mappingList = table.find(req.lpn);
...
void LAPageMapping::eraseInternal(PAL::Request &req, uint64_t &tick) {
  ...
  if (erasedCount < threshold) {
    ...
    if (!firstDeathRecorded) {
      ...
      firstDeathPrint("la_page_mapping,%" PRIu64 ",%" PRIu64 ",%" PRIu32
                      ",%" PRIu32 ",%" PRIu64 "\n",
...
float LAPageMapping::calculateWearLeveling() {
...
void LAPageMapping::calculateTotalPages(uint64_t &valid, uint64_t &invalid) {
  valid = 0;
  invalid = 0;
...
void LAPageMapping::getStatList(std::vector<Stats> &list, std::string prefix) {
  Stats temp;
...
void LAPageMapping::getStatValues(std::vector<double> &values) {
...
void LAPageMapping::resetStatValues() {
  memset(&stat, 0, sizeof(stat));
}

void LAPageMapping::printWLSummary(uint64_t tick) {
  if (!wlSummaryHeaderWritten) {
    wlSummaryPrint(
        "tick,gc_count,n_active_blocks,n_free_blocks,dead_blocks,"
        "min_ec,max_ec,mean_ec,stddev_ec,wl_factor\n");
  ...
  wlSummaryPrint("%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64
                 ",%" PRIu64 ",%" PRIu64 ",%.3f,%.3f,%.6f\n",
                 tick, stat.gcCount, nActive, nFree, nDead,
                 minEc, maxEc, meanEc, stddevEc, (double)wlFactor);
}

void LAPageMapping::printEraseCountSnapshot(uint64_t tick) {
  wlSnapshotPrint("tick,gc_count,block_idx,erase_count,state\n");

  for (auto &iter : blocks) {
    wlSnapshotPrint("%" PRIu64 ",%" PRIu64 ",%" PRIu32 ",%" PRIu32 ",active\n",
                    tick, stat.gcCount,
                    iter.first, iter.second.getEraseCount());
  }

  for (auto &iter : freeBlocks) {
    wlSnapshotPrint("%" PRIu64 ",%" PRIu64 ",%" PRIu32 ",%" PRIu32 ",free\n",
                    tick, stat.gcCount,
                    iter.getBlockIndex(), iter.getEraseCount());
  }

  for (auto &iter : deadBlocks) {
    wlSnapshotPrint("%" PRIu64 ",%" PRIu64 ",%" PRIu32 ",%" PRIu32 ",dead\n",
                    tick, stat.gcCount,
                    iter.getBlockIndex(), iter.getEraseCount());
  }
}

void LAPageMapping::printInitialSnapshot() {
  wlSnapshotInitPrint("tick,gc_count,block_idx,erase_count,state\n");

  for (auto &iter : blocks) {
    wlSnapshotInitPrint("0,0,%" PRIu32 ",%" PRIu32 ",active\n", iter.first,
                        iter.second.getEraseCount());
  }

  for (auto &iter : freeBlocks) {
    wlSnapshotInitPrint("0,0,%" PRIu32 ",%" PRIu32 ",free\n",
                        iter.getBlockIndex(), iter.getEraseCount());
  }
}
