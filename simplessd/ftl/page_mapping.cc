...
uint32_t PageMapping::getFreeBlock(uint32_t idx) {
  ...
  if (nFreeBlocks > 0) {
    ...
    printf("\n*** DEVICE END-OF-LIFE *** algo=page_mapping gc=%" PRIu64
...
void PageMapping::eraseInternal(PAL::Request &req, uint64_t &tick) {
  ...
  if (erasedCount < threshold) {
    ...
    if (!firstDeathRecorded) {
      ...
      firstDeathPrint("page_mapping,%" PRIu64 ",%" PRIu64 ",%" PRIu32
                      ",%" PRIu32 ",%" PRIu64 "\n",
...
void PageMapping::printWLSummary(uint64_t tick) {
...
void PageMapping::printEraseCountSnapshot(uint64_t tick) {
...
void PageMapping::printInitialSnapshot() {
