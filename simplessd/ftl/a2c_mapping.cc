...
std::mt19937 A2CMapping::randomChannelIndexGen;

A2CMapping::A2CMapping(ConfigReader &c, Parameter &p, PAL::PAL *l,
                       DRAM::AbstractDRAM *d)
    : AbstractFTL(p, l, d),
      pPAL(l),
      conf(c),
  ...
  channelInfoAry.resize(param.pageCountToMaxPerf, {0, 0});
  randomChannelIndexGen = std::mt19937(std::random_device{}());
  channelDist =
      std::uniform_int_distribution<uint32_t>(0, param.pageCountToMaxPerf - 1);
}

A2CMapping::~A2CMapping() {
  debugprint(LOG_FTL_PAGE_MAPPING, "===== Final FTL Statistics =====");
  debugprint(LOG_FTL_PAGE_MAPPING, "gcCount: %" PRIu64, stat.gcCount);
  debugprint(LOG_FTL_PAGE_MAPPING, "reclaimedBlocks: %" PRIu64,
             stat.reclaimedBlocks);
  debugprint(LOG_FTL_PAGE_MAPPING, "validSuperPageCopies: %" PRIu64,
             stat.validSuperPageCopies);
  debugprint(LOG_FTL_PAGE_MAPPING, "validPageCopies: %" PRIu64,
             stat.validPageCopies);
  debugprint(LOG_FTL_PAGE_MAPPING, "wearLeveling: %f", calculateWearLeveling());
  debugprint(LOG_FTL_PAGE_MAPPING, "================================");

  // 1. 인덱스와 Erase Count를 임시로 저장할 벡터 생성
  std::vector<std::pair<uint32_t, uint32_t>> blockList;
  blockList.reserve(blocks.size() + freeBlocks.size());

  // 2. 사용 중인 블록(blocks) 데이터 수집
  for (auto &iter : blocks) {
    uint32_t eraseCnt = iter.second.getEraseCount();
    uint32_t idx = iter.second.getBlockIndex();
    blockList.push_back({idx, eraseCnt});
  }

  // 3. 빈 블록(freeBlocks) 데이터 수집
  for (auto &iter : freeBlocks) {
    uint32_t eraseCnt = iter.getEraseCount();
    uint32_t idx = iter.getBlockIndex();
    blockList.push_back({idx, eraseCnt});
  }

  // 4. 인덱스(idx) 기준으로 오름차순 정렬
  std::sort(
  ...
  for (const auto &item : blockList) {
    debugprint(LOG_FTL_PAGE_MAPPING, "block[%" PRIu32 "] EraseCnt: %" PRIu32,
               item.first, item.second);
  }
}

bool A2CMapping::initialize() {
...
void A2CMapping::read(Request &req, uint64_t &tick) {
  // uint64_t begin = tick;
...
void A2CMapping::write(Request &req, uint64_t &tick) {
  // uint64_t begin = tick;
...
void A2CMapping::trim(Request &req, uint64_t &tick) {
  // uint64_t begin = tick;
...
void A2CMapping::format(LPNRange &range, uint64_t &tick) {
...
Status *A2CMapping::getStatus(uint64_t lpnBegin, uint64_t lpnEnd) {
...
float A2CMapping::freeBlockRatio() {
  return (float)nFreeBlocks / param.totalPhysicalBlocks;
}

uint32_t A2CMapping::convertBlockIdx(uint32_t blockIdx) {
  return blockIdx % param.pageCountToMaxPerf;
}

uint32_t A2CMapping::getFreeBlock(uint32_t idx) {
...
uint32_t A2CMapping::getLastFreeBlock(Bitset &iomap) {
  if (!bRandomTweak || (lastFreeBlockIOMap & iomap).any()) {
    // Update lastFreeBlockIndex
    // lastFreeBlockIndex++;
    lastFreeBlockIndex = getBestBlockIndex();
  ...
  if (freeBlock->second.getNextWritePageIndex() == param.pagesInBlock) {
    ...
    resetUsedValue(lastFreeBlockIndex);
    resetAgeValue(lastFreeBlockIndex);
...
uint32_t A2CMapping::getBestBlockIndex() {
  uint32_t index1 = getRandomChannelIndex();
  uint32_t index2 = getRandomChannelIndex();

  if (channelInfoAry[index1].usedValue == channelInfoAry[index2].usedValue) {
    if (channelInfoAry[index1].ageValue > channelInfoAry[index2].ageValue) {
      return selectAndSetChannelValue(index1, index2);
    }

    return selectAndSetChannelValue(index2, index1);
  }

  else if (channelInfoAry[index1].usedValue <
           channelInfoAry[index2].usedValue) {
    return selectAndSetChannelValue(index1, index2);
  }
...
uint32_t A2CMapping::getRandomChannelIndex() {
  return channelDist(randomChannelIndexGen);
}

uint32_t A2CMapping::selectAndSetChannelValue(uint32_t goodIndex,
                                              uint32_t badIndex) {
  addUsedValue(goodIndex);
  resetAgeValue(goodIndex);

  addAgeValue(badIndex);

  return goodIndex;
}

// calculate weight of each block regarding victim selection policy
void A2CMapping::calculateVictimWeight(
...
void A2CMapping::selectVictimBlock(std::vector<uint32_t> &list,
                                   uint64_t &tick) {
...
void A2CMapping::doGarbageCollection(std::vector<uint32_t> &blocksToReclaim,
                                     uint64_t &tick) {
  PAL::Request req(param.ioUnitInPage);
...
void A2CMapping::readInternal(Request &req, uint64_t &tick) {
...
void A2CMapping::writeInternal(Request &req, uint64_t &tick, bool sendToPAL) {
  PAL::Request palRequest(req);
...
void A2CMapping::trimInternal(Request &req, uint64_t &tick) {
  auto mappingList = table.find(req.lpn);
...
void A2CMapping::eraseInternal(PAL::Request &req, uint64_t &tick) {
...
float A2CMapping::calculateWearLeveling() {
...
void A2CMapping::calculateTotalPages(uint64_t &valid, uint64_t &invalid) {
  valid = 0;
  invalid = 0;
...
void A2CMapping::getStatList(std::vector<Stats> &list, std::string prefix) {
  Stats temp;

  temp.name = prefix + "a2c_mapping.gc.count";
  temp.desc = "Total GC count";
  list.push_back(temp);

  temp.name = prefix + "a2c_mapping.gc.reclaimed_blocks";
  temp.desc = "Total reclaimed blocks in GC";
  list.push_back(temp);

  temp.name = prefix + "a2c_mapping.gc.superpage_copies";
  temp.desc = "Total copied valid superpages during GC";
  list.push_back(temp);

  temp.name = prefix + "a2c_mapping.gc.page_copies";
  ...
  temp.name = prefix + "a2c_mapping.wear_leveling";
  temp.desc = "Wear-leveling factor";
  list.push_back(temp);
}

void A2CMapping::getStatValues(std::vector<double> &values) {
...
void A2CMapping::resetStatValues() {
  memset(&stat, 0, sizeof(stat));
}
