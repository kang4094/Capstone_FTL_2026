...
/**
 * 동적 채널 할당 (Dynamic Channel Allocation)
 */

#ifndef __FTL_DCA_MAPPING__
#define __FTL_DCA_MAPPING__
...
class DCAMapping : public AbstractFTL {
 private:
  PAL::PAL *pPAL;
  ...
  // [추가됨] 동적 채널 할당을 위한 병렬 유닛(채널/웨이 등)별 프리 블록 개수
  // 트래킹
  std::vector<uint32_t> freeBlocksPerUnit;
  ...
  // [추가됨] 프리 블록이 가장 많아 여유로운 최적의 병렬 유닛 인덱스를 탐색 및
  // 반환
  uint32_t getBestParallelUnit();
 ...
 public:
  DCAMapping(ConfigReader &, Parameter &, PAL::PAL *, DRAM::AbstractDRAM *);
  ~DCAMapping();
