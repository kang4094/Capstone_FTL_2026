...
const char NAME_WL_SUMMARY_FILE[] = "WLSummaryLogFile";
const char NAME_WL_SNAPSHOT_FILE[] = "WLSnapshotLogFile";
const char NAME_WL_SNAPSHOT_INIT_FILE[] = "WLSnapshotInitLogFile";
const char NAME_FIRST_DEATH_FILE[] = "FirstDeathLogFile";
...
bool Config::setConfig(const char *name, const char *value) {
  ...
  else if (MATCH_NAME(NAME_WL_SUMMARY_FILE)) {
    wlSummaryFile = value;
  }
  else if (MATCH_NAME(NAME_WL_SNAPSHOT_FILE)) {
    wlSnapshotFile = value;
  }
  else if (MATCH_NAME(NAME_WL_SNAPSHOT_INIT_FILE)) {
    wlSnapshotInitFile = value;
  }
  else if (MATCH_NAME(NAME_FIRST_DEATH_FILE)) {
    firstDeathFile = value;
  }
...
std::string Config::readString(uint32_t idx) {
  ...
  switch (idx) {
    ...
    case GLOBAL_LATENCY_LOG_FILE:
      ret = latencyFile;
      break;
    case GLOBAL_WL_SUMMARY_FILE:
      ret = wlSummaryFile;
      break;
    case GLOBAL_WL_SNAPSHOT_FILE:
      ret = wlSnapshotFile;
      break;
    case GLOBAL_WL_SNAPSHOT_INIT_FILE:
      ret = wlSnapshotInitFile;
      break;
    case GLOBAL_FIRST_DEATH_FILE:
      ret = firstDeathFile;
      break;
  }
