...
std::ostream *pWLSummaryFile = nullptr;
std::ofstream *pWLSnapshotFile = nullptr;
std::string wlSnapshotPathFull;
std::ostream *pWLSnapshotInitFile = nullptr;
std::ostream *pFirstDeathFile = nullptr;
...
std::ofstream wlSummaryFile;
std::ofstream wlSnapshotFile;
std::ofstream wlSnapshotInitFile;
std::ofstream firstDeathFile;
...
int main(int argc, char *argv[]) {
  ...
  std::string wlSummaryPath =
      simConfig.readString(CONFIG_GLOBAL, GLOBAL_WL_SUMMARY_FILE);
  std::string wlSnapshotPath =
      simConfig.readString(CONFIG_GLOBAL, GLOBAL_WL_SNAPSHOT_FILE);

  if (wlSummaryPath.length() != 0) {
    std::string full(argv[3]);
    joinPath(full, wlSummaryPath);
    wlSummaryFile.open(full);

    if (!wlSummaryFile.is_open()) {
      std::cerr << " Failed to open WL summary log file: " << full << std::endl;
      return 3;
    }

    pWLSummaryFile = &wlSummaryFile;
  }

  if (wlSnapshotPath.length() != 0) {
    std::string full(argv[3]);
    joinPath(full, wlSnapshotPath);
    wlSnapshotFile.open(full);

    if (!wlSnapshotFile.is_open()) {
      std::cerr << " Failed to open WL snapshot log file: " << full
                << std::endl;
      return 3;
    }

    pWLSnapshotFile = &wlSnapshotFile;
    wlSnapshotPathFull = full;
  }

  std::string wlSnapshotInitPath =
      simConfig.readString(CONFIG_GLOBAL, GLOBAL_WL_SNAPSHOT_INIT_FILE);
  std::string firstDeathPath =
      simConfig.readString(CONFIG_GLOBAL, GLOBAL_FIRST_DEATH_FILE);

  if (wlSnapshotInitPath.length() != 0) {
    std::string full(argv[3]);
    joinPath(full, wlSnapshotInitPath);
    wlSnapshotInitFile.open(full);

    if (!wlSnapshotInitFile.is_open()) {
      std::cerr << " Failed to open WL initial snapshot log file: " << full
                << std::endl;
      return 3;
    }

    pWLSnapshotInitFile = &wlSnapshotInitFile;
  }

  if (firstDeathPath.length() != 0) {
    std::string full(argv[3]);
    joinPath(full, firstDeathPath);
    firstDeathFile.open(full);

    if (!firstDeathFile.is_open()) {
      std::cerr << " Failed to open first-death log file: " << full
                << std::endl;
      return 3;
    }

    pFirstDeathFile = &firstDeathFile;
  }

  // Initialize SimpleSSD
  auto ssdConfig = initSimpleSSDEngine(&engine, pDebugLog, pDebugLog, argv[2]);

  SimpleSSD::setWLFiles(pWLSummaryFile, pWLSnapshotFile, wlSnapshotPathFull,
                        pWLSnapshotInitFile, pFirstDeathFile);
